...
const SuperAdminSidebar = ({ collapsed = false }: SuperAdminSidebarProps) => {
    ...
    return (
        <aside className={`${collapsed ? 'w-16' : 'w-[260px]'} bg-[#262255] border-r border-[#262255]/20 text-slate-200 h-screen fixed left-0 top-0 flex flex-col z-50 transition-all duration-300` }>
            {/* Brand Section */}
            <div className={`${collapsed ? 'p-4' : 'p-8'} border-b border-white/10`}> 
                ...
                {menuItems.map((item, index) => {
                    ...
                    return (
                        <NavLink
                            ...
                            className={({ isActive }) =>
                                `flex items-center ${collapsed ? 'justify-center px-2' : 'gap-3 px-4'} py-3 rounded-xl transition-all duration-200 group relative ${isActive
                        ...
                        >
                            {({ isActive }) => (
                                <>
                                    ...
                                    <span className={`font-medium text-sm tracking-tight transition-all duration-300 ${collapsed ? 'opacity-0 w-0 hidden' : 'opacity-100'}`}>{item.name}</span>
                                    {isActive && !collapsed && (
            ...
            <div className="p-4 border-t border-white/10">
                <div className={`bg-[#2A2760] rounded-2xl p-4 flex items-center gap-3 border border-white/5 transition-all ${collapsed ? 'justify-center' : ''}`}>
                    ...
                    <div className={`flex-1 min-w-0 transition-all ${collapsed ? 'hidden' : ''}`}>
                    ...
                    <button
                        ...
                        className={`p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all ${collapsed ? 'hidden' : ''}`}
                        title="Logout"
                    >
                        <LogOut size={18} />
                    </button>
