  ...
  if (loading) {
    return (
      <div className="bg-white rounded-[32px] p-24 flex flex-col items-center justify-center border border-slate-100 shadow-sm transition-all duration-300">
        ...
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[2px] animate-pulse mt-6">Indexing Logs...</p>
      </div>
    )
  }
  ...
  if (movements.length === 0) {
    return (
      <div className="bg-white rounded-[32px] p-24 flex flex-col items-center justify-center border border-slate-100 shadow-sm text-center">
        ...
        <h3 className="text-xl font-black text-slate-900 tracking-tight mb-2">No logs recorded</h3>
    ...
    <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden animate-fade-in hover:shadow-lg transition-all duration-300">
      ...
      <div className="px-8 py-6 border-t border-slate-50 flex items-center justify-between">
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
          Showing <span className="text-slate-900 font-extrabold">1</span> to <span className="text-slate-900">{Math.min(5, movements.length)}</span> of <span className="text-slate-900">{movements.length}</span> entries
        </p>
        <div className="flex items-center gap-1.5">
          ...
          <button className="w-10 h-10 rounded-xl bg-[#1E1B4B] text-white font-black text-[10px] shadow-md shadow-[#1E1B4B]/20 border border-[#1E1B4B]/20">
            1
          </button>
          <button className="w-10 h-10 rounded-xl bg-white border border-slate-100 text-slate-400 text-[10px] font-black hover:bg-slate-50">
            2
          </button>
