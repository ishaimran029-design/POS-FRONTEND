...
const SalesRow: React.FC<Props> = ({ transaction, onCancel, onRefund }) => {
  ...
  return (
    <tr className="border-b hover:bg-[#2563EB]/5 transition-all duration-300 group cursor-pointer">
      ...
      <td className="p-4">
        <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full bg-[#2563EB]/5 text-[#2563EB] border border-[#2563EB]/10 flex items-center justify-center text-xs font-black uppercase">
                {transaction.customer && transaction.customer !== "Guest" ? transaction.customer.charAt(0) : "G"}
            </div>
            <span className="font-medium text-gray-800 group-hover:text-[#1E1B4B] transition-colors">{transaction.customer || "Guest"}</span>
        </div>
      </td>
      ...
      <td className="p-4 relative" ref={menuRef}>
        <button 
          ...
          className="text-slate-400 hover:text-[#2563EB] transition-colors p-2 rounded-xl hover:bg-white border border-transparent hover:border-[#2563EB]/10 shadow-sm"
        >
        ...
        {menuOpen && (
          <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-100 rounded-lg shadow-lg z-10 py-1 font-medium text-sm overflow-hidden">
            <button 
              ...
              className="w-full text-left px-4 py-2 hover:bg-[#2563EB]/5 transition-colors text-slate-700 hover:text-rose-600 text-xs font-bold"
            >
              Cancel Sale
            </button>
            <button 
              onClick={() => { setMenuOpen(false); onRefund(); }}
              className="w-full text-left px-4 py-2 hover:bg-[#2563EB]/5 transition-colors text-slate-700 hover:text-amber-600 text-xs font-bold"
            >
              Refund Sale
            </button>
          </div>
