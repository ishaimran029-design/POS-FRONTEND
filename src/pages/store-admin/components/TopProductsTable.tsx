    ...
    return (
        <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="pl-12 pr-4 py-2 bg-slate-100/50 border-none rounded-2xl text-sm focus:ring-2 focus:ring-indigo-100 transition-all placeholder:text-slate-400 w-full sm:w-48"
                        />
                    </div>
                                ...
                                <td className="px-8 py-5 min-w-[180px]">
                                    <div className="space-y-1.5">
                                        <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                                            <span className={product.stockLevel < 30 ? 'text-rose-500' : 'text-slate-400'}>
                                                {product.stockLevel < 30 ? 'Critical' : 'Good'}
                                            </span>
                                            <span className="text-slate-900">{product.stockLevel}%</span>
                                        </div>
                                        <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                            <div
                                                className={`h-full rounded-full transition-all duration-1000 ${product.stockLevel < 30 ? 'bg-rose-500' :
                                                    product.stockLevel < 60 ? 'bg-amber-500' : 'bg-emerald-500'
                                                    }`}
            ...
            <div className="p-6 bg-slate-50/50 flex justify-center border-t border-slate-50">
                <button className="text-xs font-black text-indigo-600 uppercase tracking-widest hover:underline px-4 py-2">
